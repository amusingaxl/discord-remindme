...
export default {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const reminders = await database.getUserReminders(interaction.user.id, includeCompleted);

            if (reminders.length === 0) {
                const embed = new EmbedBuilder()
                    .setColor('#ffaa00')
                    .setTitle('📭 No Reminders')
                    .setDescription(includeCompleted 
            ...
            const embed = new EmbedBuilder()
                .setColor('#0099ff')
                .setTitle(includeCompleted ? '✅ Completed Reminders' : '📋 Your Active Reminders');

            const activeReminders = reminders.filter(r => !r.is_completed);
            const completedReminders = reminders.filter(r => r.is_completed);

            if (!includeCompleted && activeReminders.length > 0) {
                const reminderList = activeReminders.slice(0, 10).map(reminder => {
                    const timeFormatted = TimeParser.formatReminderTime(
                        new Date(reminder.scheduled_time), 
                        userRecord.timezone
                    );
                    
                    const targetInfo = reminder.target_user_id && reminder.target_user_id !== reminder.user_id
                ...
                if (activeReminders.length > 10) {
                    embed.setFooter({ text: `Showing 10 of ${activeReminders.length} reminders` });
                }
            } else if (includeCompleted && completedReminders.length > 0) {
                const reminderList = completedReminders.slice(0, 10).map(reminder => {
                    const completedTime = new Date(reminder.scheduled_time).toLocaleDateString();
                    const targetInfo = reminder.target_user_id && reminder.target_user_id !== reminder.user_id
                        ? ` → <@${reminder.target_user_id}>`
                        : '';
                ...
                if (completedReminders.length > 10) {
                    embed.setFooter({ text: `Showing 10 of ${completedReminders.length} completed reminders` });
                }
            }
